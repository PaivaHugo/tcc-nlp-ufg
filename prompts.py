...
class BasePrompt():
    def __init__(self):
        ...
        - Analisar a chave "scores" do JSON recebido, onde encontram-se as preferências temáticas do usuário.
        ...
        Preferências temáticas do usuário:
        {{
            "nome": "Camila",
            "id": 1,
            "scores": {{
...
class MaritacaPrompt(BasePrompt):
    ...
    def get(self, news_content, user_preferences):
        template = """
            {system_prompt}
            Conteúdo completo da notícia: {news}
            Preferências temáticas do usuário: {user_preferences}
        """

        prompt = PromptTemplate.from_template(template.format(system_prompt=self.system_prompt, news=news_content, user_preferences=user_preferences))
